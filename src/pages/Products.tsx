...
const Products = () => {
  ...
  }, [selectedIds, bulkCategoryId, bulkUnitId, productsQuery]);
  ...
  const handleImport = useCallback(
    ...
    [business, categoriesQuery.data, brandsQuery.data, unitsQuery.data, productsQuery],
  );
  ...
  return (
    <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {brandsQuery.data?.map((b) => (
                  <Badge key={b.id} variant="outline" className="gap-1 py-1.5 px-3 text-sm">
                    {b.name}
                    <button
                      onClick={() => removeBrand.mutate(b.id)}
                      className="ml-1 text-muted-foreground hover:text-destructive"
                    >
                      <Trash2 className="h-3 w-3" />
                    </button>
      ...
      <QuickAddDialog
        ...
        onSubmit={(name) => createUnit.mutate({ name })}
...
const ProductRow = memo(function ProductRow({
  ...
  const margin =
    p.purchase_price > 0 ? (((p.selling_price - p.purchase_price) / p.purchase_price) * 100).toFixed(0) : "—";
  ...
  return (
    <TableRow className={isSelected ? "bg-muted/50" : ""}>
      ...
      <TableCell className="font-medium">
        <button onClick={() => {}} className="text-left hover:text-primary hover:underline">
          {p.name}
        </button>
      </TableCell>
